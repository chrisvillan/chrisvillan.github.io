...
// ============================================================
// POLL ACCESS SETTINGS
// ============================================================

async function getPollSettings(env) {
  const row =
    await env.POLL_DB
      .prepare(`
        SELECT
  ...
  return {
    accessMode:
      row?.access_mode === "live"
        ? "live"
        : "session",

    liveSessionId:
      row?.live_session_id
        ? String(row.live_session_id)
        : null
  };
}


async function getPollBootstrap(env) {
  const settings =
    await getPollSettings(env);

  if (
    settings.accessMode === "live"
  ) {
    const liveSession =
      await getPollSessionById(
        env,
        settings.liveSessionId
      );
...
async function loadLivePollState(env) {
  const settings =
    await getPollSettings(env);
  ...
  if (!session) {
    throw new Error(
      "No live session has been selected."
    );
  }
...
async function savePollSettings(
  ...
  if (
    ...
    if (!liveSessionId) {
      throw new Error(
        "Choose a session for Live Mode."
      );
    }
    ...
    if (!session) {
      throw new Error(
        "The selected live session could not be found."
      );
    }
  } else {
    liveSessionId = null;
...
async function getPollSessionById(
  env,
  sessionId
) {
  sessionId =
